...
LINEAR_PB = (
    True
)  # if True solves linear Poisson-Boltzmann, otherwise solves nonlinear PB.
...
e2_per_Angs_to_kcal_per_mol = (
    e_C ** 2 * N_A * 1e8 / erg_per_kcal
)  # 1 esu^2/A = 332.06364 kcal/mol

# 1 erg = 1e-8 esu^2/Angstrom -> esu/erg = 1e8 Angstrom/esu

KbT_in_kcal_per_mol = (
    N_A * Kb * T / erg_per_kcal
)  # in kcal/mol; KbT  = 0.592183 * (T/298.0)  in kcal/mol


kappa_sq_in_angs2 = (
    1e-16 * (8 * PI * e_C ** 2 * N_A / (1000 * Kb * T)) * ionic_strength / eps_p_r
)  # this is in units of Angstrom^-2
kappa_bar_sq_p = (
    kappa_sq_in_angs2 * eps_p_r
)  # equivalent to 8.486902807 * ionic_strength in Angstrom^-2
kappa_bar_sq_m = 0.0
...
rhs_esu2_per_erg = (
    4 * PI * e_C ** 2 / (Kb * T)
)  # in esu^2/erg; useful in sum{ 4*PI*e_C**2/(Kb*T) z_i * delta(x-x_i)}
