...
from jax_dips.jaxmd_modules.util import Array
from jax_dips.jaxmd_modules.util import f32
from jax_dips.jaxmd_modules.util import f64
from jax_dips.jaxmd_modules.util import safe_mask
...
def periodic(side: Box, wrapped: bool = True) -> Space:
    ...
    def displacement_fn(Ra: Array, Rb: Array, **unused_kwargs) -> Array:
        if "box" in unused_kwargs:
            raise ValueError(
                (
                    "`space.periodic` does not accept a box argument."
