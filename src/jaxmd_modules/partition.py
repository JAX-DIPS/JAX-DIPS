...
def cell_list(
    ...
    def build_cells(R, **kwargs):
        ...
        cell_R = cell_R.at[sorted_cell_id].set(
            sorted_R
        )  # ops.index_update(cell_R, sorted_cell_id, sorted_R)
        sorted_id = np.reshape(sorted_id, (N, 1))
        cell_id = cell_id.at[sorted_cell_id].set(
            sorted_id
        )  # ops.index_update(cell_id, sorted_cell_id, sorted_id)
...
class NeighborList(object):
    ...
    cell_list_fn: Callable[
        [Array], CellList
    ] = src.jaxmd_modules.dataclasses.static_field()
    ...
    **static_kwargs
    ...
    def candidate_fn(R, **kwargs):
        return np.broadcast_to(
            np.reshape(np.arange(R.shape[0]), (1, R.shape[0])), (R.shape[0], R.shape[0])
        )
    ...
    def neighbor_list_fn(
        ...
        **kwargs
        ...
        if nbrs is None:
            cell_list_fn = (
                cell_list(box_size, cell_size, R, capacity_multiplier)
                if use_cell_list
                else None
            )
