...
First order methods are slow but cheap; second order methods are fast but expensive.


\subsubsection{First order optimization methods}
Only use Gradient:
\begin{itemize}
\item Adam
\item RMSProp
...
\subsubsection{Second order optimization methods}
Explicitly compute Hessian:
\begin{itemize}
\item Newton method
...
\subsubsection{Conjugate directions optimization methods}
Build a basis vectors by computing gradients that are conjugate to each other $\mathbf{p}_j^T A \mathbf{p}_i=0$. They are somewhere between first order and second order optimization methods.

Applicable to quadratic problems such as the one appearing here $\min_{x\in R^n} \frac{1}{2} x^T A x - b^T x$ (equivalent to solving $Ax=b$), assuming $A$ is symmetric positive definite then CG method can solve this problem in at most $n$ steps.
