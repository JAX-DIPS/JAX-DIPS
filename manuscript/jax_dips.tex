...
The linear system suffers from worse condition number in the domain with more variability in diffusion coefficient, or where diffusion coefficient is larger. This leads to regionally imbalanced solution error where the overall error is systematically dominated by the bigger-diffusion region. 

We found this problem can be drastically improved by interweaving region-specific optimization epochs inside the training pipeline. See algorithm \ref{switching}.
	...
	\begin{algorithmic}[1]
		...
		\For {$epoch$ in $0\cdots$ N}		
			\If {$epoch\ \%\ \tau > 0$}
				...
				\If {$\mu^- > \mu^+$}
					\State optimize $u^-_{NN}$ in $\Omega^- \cup  \Gamma_{extrapolation\ region}$ 
				\Else
					\State optimize $u^+_{NN}$ in $\Omega^+ \cup \Gamma_{extrapolation\ region}$ 
				\EndIf		
