...
\item FD kernel is applied on the compute cell where the solution values are evaluated by the neural network. Each kernel contributes a linear system residual $r_p = ||A u_p - b||$ at a point $p$.

\end{enumerate}

We convolve the complex discretization kernels over neural network values evaluated at structured compact stencils, 
